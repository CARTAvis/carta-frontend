...
const decremental: Point2D[] = [{x: 20, y: 0}, {x: 17, y: 0}, {x: 9, y: 0}, {x: 7, y: 0}, {x: 6.5, y: 0}, {x: 4, y: 0}, {x: 3, y: 0}, {x: 2, y: 0}, {x: 1, y: 0}, {x: -15, y: 0}];
...
test("test binarySearchByX with sorted array in incremental/decremental order", () => {
    ...
    expect(binarySearchByX(incremental, 2048)).toEqual({point: {x: 1074, y: 0}, index: 8});

    expect(binarySearchByX(decremental, 999)).toEqual({point: {x: 20, y: 0}, index: 0});
    expect(binarySearchByX(decremental, 5)).toEqual({point: {x: 6.5, y: 0}, index: 4});
    expect(binarySearchByX(decremental, 0)).toEqual({point: {x: 1, y: 0}, index: 7});
    expect(binarySearchByX(decremental, -33)).toEqual({point: {x: -15, y: 0}, index: 8});
});
