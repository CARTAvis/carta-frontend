...
describe("test zIndexManager", () => {
    test("return expected zIndex order", () => {
        ...
        mockObjsId.map(w => zIndexManager.assignIndex(w));

        expect(zIndexManager.getFloatingObjs().length).toBe(mockObjsId.length);
        let expectOrder = [1, 2, 3, 4];
        for (let i = 1; i < mockObjsId.length; i++) {
        ...
        expect(zIndexManager.getFloatingObjs().length).toBe(mockObjsId.length - 1);
