...
export class LayoutStore {
    ...
    @flow.bound *fetchLayouts() {
        try {
            ...
            AlertStore.Instance.shouldShowDashboard("Loading user-defined layout failed!");
            console.error(err);
        }
    }
    ...
    @action applyLayout = (layoutName: string): boolean => {
        if (!layoutName || !this.layoutExists(layoutName)) {
            AlertStore.Instance.shouldShowDashboard(`Applying layout failed! Layout ${layoutName} not found.`);
            return false;
        }
    ...
    @flow.bound *saveLayout() {
        ...
        if (!this.layouts || !this.layoutNameToBeSaved || !this.dockedLayout) {
            appStore.alertStore.shouldShowDashboard("Save layout failed! Empty layouts or name.");
            return;
        }
        ...
        if (!this.layoutExists(this.layoutNameToBeSaved) && this.numSavedLayouts >= MAX_LAYOUT) {
            appStore.alertStore.shouldShowDashboard(`Maximum user-defined layout quota exceeded! (${MAX_LAYOUT} layouts)`);
            return;
        }

        const currentConfig = this.dockedLayout.toConfig();
        if (!currentConfig || !currentConfig.content || currentConfig.content.length <= 0) {
            appStore.alertStore.shouldShowDashboard("Saving layout failed! Something is wrong with current layout.");
            return;
        }

        const configToSave = LayoutConfig.CreateConfigToSave(appStore, currentConfig.content[0]);
        if (!configToSave) {
            appStore.alertStore.shouldShowDashboard("Saving layout failed! Creat layout configuration for saving failed.");
            return;
        }
    ...
    private handleSaveResult = (isSuccess: boolean) => {
        if (isSuccess) {
            ...
            AlertStore.Instance.shouldShowDashboard("Saving user-defined layout failed! ");
        }
    };
    ...
    @flow.bound *renameLayout(oldName: string, newName: string) {
        ...
        if (this.layoutExists(newName)) {
            appStore.alertStore.shouldShowDashboard("Layout name already exists.");
            return;
        }

        if (!oldName || !this.layoutExists(oldName)) {
            appStore.alertStore.shouldShowDashboard(`Cannot rename layout ${oldName}! It does not exist.`);
            return;
        }
    ...
    private handleRenameResult = (oldName: string, newName: string, isSuccess: boolean) => {
        if (isSuccess) {
            ...
            AlertStore.Instance.shouldShowDashboard("Renaming user-defined layout failed!");
        }
    };
    ...
    @flow.bound *deleteLayout(layoutName: string) {
        ...
        if (!layoutName || !this.layoutExists(layoutName)) {
            appStore.alertStore.shouldShowDashboard(`Cannot delete layout ${layoutName}! It does not exist.`);
            return;
        }
    ...
    private handleDeleteResult = (layoutName: string, isSuccess: boolean) => {
        if (isSuccess) {
            ...
            AlertStore.Instance.shouldShowDashboard("Deleting user-defined layout failed!");
        }
    };
}
