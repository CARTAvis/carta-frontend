...
export class ImageFittingStore {
    ...
    @computed get isValidParams() {
        return this.isAutoInitVal ? true : this.components.every(c => c.isValidParams === true);
    }
    ...
    @computed get isFitDisabled() {
        ...
        return !isValidFileId || isAllFixed || !this.isValidParams || this.isFitting;
    }
    ...
    setResultString = (
        ...
        const isShowIntegratedFlux = integratedFluxValues.length === values.length && integratedFluxErrors.length === values.length && (frame.requiredUnit === "Jy/pixel" || frame.requiredUnit === "Jy/beam");
...
class ImageFittingIndividualStore {
    ...
    @computed get isValidParams(): boolean {
