...
describe("scaling parameter configuration", () => {
    ...
    test.each([
        {scaling: FrameScaling.LOG, min: 0.1, max: 1_000_000, defaultValue: 1_000, preferenceKey: PreferenceKeys.RENDER_CONFIG_SCALING_ALPHA_LOG},
        {scaling: FrameScaling.GAMMA, min: 0.01, max: 10, defaultValue: 0.3, preferenceKey: PreferenceKeys.RENDER_CONFIG_SCALING_GAMMA},
        {scaling: FrameScaling.POWER, min: 0.000001, max: 1_000_000, defaultValue: 0.01, preferenceKey: PreferenceKeys.RENDER_CONFIG_SCALING_ALPHA_POWER},
    ...
    test("clamps finite values and replaces non-finite values", () => {
        ...
        expect(sanitizeScalingParameter(FrameScaling.LOG, 1e300)).toBe(1_000_000);
        ...
        expect(sanitizeScalingParameter(FrameScaling.GAMMA, 1e-300)).toBe(0.01);
    ...
    test.each([
        ...
        {scaling: FrameScaling.LOG, alpha: 1_000_000},
    ...
    test.each([0.01, 10])("keeps the shader-equivalent float32 Gamma transform accurate at gamma %s", gamma => {
        for (const x of TEST_SAMPLES) {
            expect(shaderScaleValue(x, FrameScaling.GAMMA, gamma)).toBeCloseTo(scaleValue(x, FrameScaling.GAMMA, 1, gamma), 6);
        }
    });

    test.each([0.000001, 1.1, 10, 1_000_000])("keeps the shader-equivalent float32 Power transform accurate at alpha %s", alpha => {
        for (const x of TEST_SAMPLES) {
