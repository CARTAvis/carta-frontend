...
const CreateComponentHarness = (system: CatalogSystemType, columns: MockColumn[], xAxis: string = CatalogOverlay.NONE, yAxis: string = CatalogOverlay.NONE, options: {autoSelectEnabled?: boolean; displayStore?: MockDisplayStore} = {}) => {
...
const CreateComponentWithoutProfileStore = (xAxis: string = CatalogOverlay.NONE, yAxis: string = CatalogOverlay.NONE) => {
    harnessId += 1;
    const component = Object.create(CatalogOverlayComponent.prototype) as CatalogOverlayComponent & Record<string, any>;
    const displayStore = CreateDisplayStore(xAxis, yAxis);
    component["catalogFileNames"] = new Map<number, string>();
    component["widgetId"] = `catalog-overlay-test-${harnessId}`;
...
describe("CatalogOverlayComponent", () => {
    describe("autoSelectAxes", () => {
        test.each([
            ...
            const {component, displayStore} = CreateComponentHarness(system, columns);
        ...
        test.each([
            ...
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, columns);
        ...
        test("skips excluded coordinate-like error columns", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "e_ra"}, {name: "pmdec"}, {name: "ra"}, {name: "dec"}]);
        ...
        test("skips columns when data type metadata is missing", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "ra"}, {name: "dec"}]);
            ...
            expect(displayStore.setxAxis).not.toHaveBeenCalled();
        ...
        test("prefers FK4 columns over explicit J2000 or ICRS columns", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.FK4, [{name: "RAJ2000"}, {name: "DEJ2000"}, {name: "RA_ICRS"}, {name: "DE_ICRS"}, {name: "RAB1950"}, {name: "DEB1950"}]);
        ...
        test("prefers FK5 columns over explicit B1950 or ICRS columns", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.FK5, [{name: "RAB1950"}, {name: "DEB1950"}, {name: "RA_ICRS"}, {name: "DE_ICRS"}, {name: "RAJ2000"}, {name: "DEJ2000"}]);
        ...
        test("prefers ICRS columns and excludes explicit B1950 columns", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "RAB1950"}, {name: "DEB1950"}, {name: "RAJ2000"}, {name: "DEJ2000"}, {name: "RA_ICRS"}, {name: "DE_ICRS"}]);
        ...
        test("only treats J2000 FK5 columns as ICRS-compatible", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "RAJ2021"}, {name: "DEJ2021"}, {name: "RAJ2000"}, {name: "DEJ2000"}]);
        ...
        test.each([
            ...
            const {component, displayStore} = CreateComponentHarness(system, [{name: incompatibleX}, {name: incompatibleY}, {name: compatibleX}, {name: compatibleY}]);
        ...
        test("enables hidden matching columns when no visible coordinate columns are available", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(CatalogSystemType.Pixel0, [{name: "flux"}, {name: "xcentroid", display: false}, {name: "ycentroid", display: false}]);
        ...
        test("does nothing when preference is disabled", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(
        ...
        test("force-reset path reselects columns for the new system", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "ra"}, {name: "dec"}, {name: "X_IMAGE"}, {name: "Y_IMAGE"}], "ra", "dec");
            ...
            expect(displayStore.setxAxis).toHaveBeenNthCalledWith(1, CatalogOverlay.NONE);
            expect(displayStore.setyAxis).toHaveBeenNthCalledWith(1, CatalogOverlay.NONE);
    ...
    describe("auto-select axes reaction", () => {
        ...
        test("defers attempt tracking until ImageOverlay mode is active", () => {
            const {displayStore} = CreateConstructedComponentHarness(CatalogSystemType.ICRS, [{name: "ra"}, {name: "dec"}], {catalogPlotType: CatalogPlotType.Histogram});
        ...
        test("does not retry auto-selection when another component uses the same widget store", () => {
            ...
            expect(firstHarness.displayStore.xAxis).toBe("ra");
            expect(firstHarness.displayStore.yAxis).toBe("dec");

            firstHarness.displayStore.setxAxis(CatalogOverlay.NONE);
            firstHarness.displayStore.setyAxis(CatalogOverlay.NONE);
            ...
            CreateConstructedComponentHarness(CatalogSystemType.ICRS, [{name: "ra"}, {name: "dec"}], {
                ...
                displayStore: firstHarness.displayStore
            });

            expect(firstHarness.displayStore.xAxis).toBe(CatalogOverlay.NONE);
            expect(firstHarness.displayStore.yAxis).toBe(CatalogOverlay.NONE);
            expect(firstHarness.displayStore.hasAttemptedAutoSelectImageOverlayAxes).toBe(true);
        });
    });
    ...
    describe("handleHeaderDisplayChange", () => {
        ...
        test("reselects visible coordinate axes when columns are toggled back on from None", () => {
            const {component, displayStore} = CreateComponentHarness(
                CatalogSystemType.ICRS,
                [
    ...
    describe("handleHeaderDisplayChange reselects removed axes", () => {
        ...
        test("only reselects the removed xAxis", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.Pixel0, [{name: "x"}, {name: "y"}, {name: "xcentroid"}], "x", "y");
        ...
        test("only reselects the removed yAxis", () => {
            const {component, displayStore} = CreateComponentHarness(CatalogSystemType.Pixel0, [{name: "x"}, {name: "y"}, {name: "ycentroid"}], "x", "y");
    ...
    describe("handleCatalogSystemChange", () => {
        test("clears image overlay axes when preference is disabled and the axis labels change", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(CatalogSystemType.ICRS, [{name: "ra"}, {name: "dec"}, {name: "X_IMAGE"}, {name: "Y_IMAGE"}], "ra", "dec", {autoSelectEnabled: false});
        ...
        test("preserves image overlay axes when preference is disabled and the axis labels stay compatible", () => {
            const {component, profileStore, displayStore} = CreateComponentHarness(CatalogSystemType.FK5, [{name: "_RAJ2000"}, {name: "_DEJ2000"}], "_RAJ2000", "_DEJ2000", {autoSelectEnabled: false});
