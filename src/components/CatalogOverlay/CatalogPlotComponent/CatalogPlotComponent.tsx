...
export class CatalogPlotComponent extends React.Component<WidgetProps> {
    ...
    constructor(props: WidgetProps) {
        ...
        reaction(
            ...
            () => {
                if (this.widgetStore?.shouldEnableStatistic) {
    ...
    @action handleCatalogFileChange = (fileId: number) => {
        ...
        if (plotWidgetStoreId) {
            ...
            const isXColumn = plotWidgetStore?.xColumnName === CatalogPlotComponent.emptyColumn;
            ...
            switch (plotWidgetStore?.plotType) {
                ...
                case CatalogPlotType.Histogram:
                    if (!isXColumn && plotWidgetStore.histogramBorder === undefined) {
                        const xColumnName = plotWidgetStore.xColumnName;
                        if (xColumnName) {
    ...
    private updateStatistic = () => {
        const profileStore = this.profileStore;
        const widgetStore = this.widgetStore;
        if (!widgetStore?.shouldEnableStatistic || !profileStore || !widgetStore.statisticColumnName) {
            return;
        }
    ...
    public render() {
        ...
        if (widgetStore.shouldShowStatisticResult && widgetStore.shouldEnableStatistic) {
                        ...
                        <div className={Classes.DIALOG_FOOTER_ACTIONS}>
                            <Tooltip content={"Show only selected sources at image and table viewer"}>
                                <FormGroup label={"Selected only"} inline={true} disabled={isDisabled}>
                                    <Switch checked={catalogWidgetStore.isShowSelectedData} onChange={this.handleShowSelectedDataChanged} disabled={isDisabled} />
                                </FormGroup>
                            </Tooltip>
