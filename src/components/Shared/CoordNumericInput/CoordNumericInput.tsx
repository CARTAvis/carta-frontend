...
const WcsCoordNumericInput = ({inputType, valueWcs, onChangeWcs, disabled = false, customPlaceholder = ""}: WcsCoordNumericInputProps) => {
    ...
    switch (inputType) {
        case InputType.XCoord:
            const formatX = AppStore.Instance.imageViewConfigStore.visibleFrames[0].overlayStore.numbers.formatTypeX;
            tooltipContent = `Format: ${NUMBER_FORMAT_LABEL.get(formatX)}`;
            break;
        case InputType.YCoord:
            const formatY = AppStore.Instance.imageViewConfigStore.visibleFrames[0].overlayStore.numbers.formatTypeY;
