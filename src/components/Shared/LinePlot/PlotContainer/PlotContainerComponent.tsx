...
export class PlotContainerComponent extends React.Component<PlotContainerProps> {
    ...
    private filterLogTicks = (axis: Scale) => {
        const ticks = this.isRemoveAdditionalTicks(axis.ticks);
    ...
    private filterLinearTicks = (axis: Scale, isRemoveAdditionalTicks: boolean = true) => {
        let isRemoveFirstTick = false;
        let isRemoveLastTick = false;
        let roundingDecimalDigits: number | undefined;
        const ticks = isRemoveAdditionalTicks ? this.isRemoveAdditionalTicks(axis.ticks) : axis.ticks;
        ...
        const newticks = ticks.slice(isRemoveFirstTick ? 1 : 0, isRemoveLastTick ? -1 : undefined);
    ...
    private isRemoveAdditionalTicks = (ticks: Tick[]) => {
    ...
    private skipLinearLongTicks = (axis: Scale) => {
        if (axis.ticks && axis.ticks.length >= 4) {
            ...
            if (interTickPixelDist < (firstTickLabel?.length || 0) * 6 + 5) {
                ...
                const isSkipOdd = axis.ticks.length % 2 === 0;
                for (let i = isSkipOdd ? 1 : 0; i < axis.ticks.length; i = i + 2) {
