...
export class RegionSelectorComponent extends React.Component<{widgetStore: RegionWidgetStore; onFrameChanged?: (newFrame: FrameStore) => void}> {
    ...
    public render() {
        ...
        let isEnableFrameselect = false;
        let selectedFrameValue: number = ACTIVE_FILE_ID;
        if (appStore.activeFrame) {
            selectedFrameValue = widgetStore.fileId;
            isEnableFrameselect = true;
        }

        let isEnableRegionSelect = false;
        ...
        return (
            <React.Fragment>
                <FormGroup label={"Image"} inline={true} disabled={!isEnableFrameselect}>
                    <HTMLSelect
                        className={frameClassName}
                        value={selectedFrameValue}
                        options={widgetStore.frameOptions}
                        onChange={this.handleFrameChanged}
                        disabled={!isEnableFrameselect}
                        style={{width: "100px"}}
                        data-testid="image-dropdown"
                    />
                </FormGroup>
                <FormGroup label={"Region"} inline={true} disabled={!isEnableRegionSelect}>
                    <HTMLSelect className={regionClassName} value={selectedValue} options={regionOptions} onChange={this.handleRegionChanged} disabled={!isEnableRegionSelect} data-testid="region-dropdown" />
                </FormGroup>
            </React.Fragment>
        );
    }
