...
export class PolygonRegionForm extends React.Component<{region: RegionStore; wcsInfo: AST.FrameSet}> {
    ...
    public render() {
        ...
        const pointRows = region.controlPoints.map((point, index) => {
            ...
            const infoString = region.coordinate === CoordinateMode.Image ? `WCS: ${isImgCoordinates ? "-" : pointWCS ? WCSPoint2D.toString(pointWCS, 3) : ""}` : `Image: ${Point2D.toString(point, "px", 3)}`;
            return (
