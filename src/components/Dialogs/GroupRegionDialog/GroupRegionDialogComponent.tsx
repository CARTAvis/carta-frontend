...
export class GroupRegionDialogComponent extends React.Component {
    private static readonly MissingRegionNode = (<NonIdealState icon={"folder-open"} title={"No regions selected"} description={"Select multiple regions using Shift+click"} />);
    ...
    private handleDeleteClicked = async () => {
    ...
    public render() {
        ...
        const selectedCount = activeFrame?.regionSet.selectedRegionsList.length ?? 0;
        const editableRegion = !!primaryRegion && selectedCount > 1;
        const selectedRegions = activeFrame?.regionSet.selectedRegionsList ?? [];
        const allLocked = selectedRegions.length > 0 && selectedRegions.every(r => r.locked);
        const anyVisible = selectedRegions.some(r => r.visible);
        ...
        if (editableRegion && activeFrame) {
            dialogProps.title = `Editing Group Region (${activeFrame.filename})`;
