...
export const ONLINE_CATALOG_QUERY_HELP_CONTENT = (
    <div>
        ...
        <p>
            ...
            manually or use the <code>Set center</code> button (the "target" icon) to set the center coordinate in the active image view as the center to search for source catalogs. The search radius can be defined with the text field or
            with the <code>Set to view</code> button which adopts the field of view of the active image view.
        </p>
        <p>
            If the database is VizieR, addtionally you may supply catalog tiles in the <code>Keywords</code> field for the query. After you have clicked the <code>Query</code> button, a set of catalog <em>titles</em> will be displayed once
            you click the <code>VizieR catalog</code> field. You can then select the desired catalog titles (multi-selection) and click the <code>Load selected</code> button to retrieve corresponding source catalogs.
        </p>
        <p>
