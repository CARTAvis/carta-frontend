...
export class ImageViewHelpComponent extends React.Component {
    public render() {
        return (
            <div>
                <p>The image viewer widget serves as the core compoment of CARTA. It allows users to visualize images in rasters and in contours. Region of interests can be defined interactively with the image viewer and subsequent image analysis can be performed with other widgets.</p>
                <p>Images can be loaded via <strong>File</strong> -&gt; <strong>Open image</strong> (will close all loaded image first) If users may load multiple images via  <strong>File</strong> -&gt; <strong>Append image</strong>. All images are loaded as raster by default. Contour layers can be further generated via the contour configuration dialogue.</p>
                ...
                <p><img src={imageTools} style={{ width: "70%", height: "auto" }}/></p>
                    <h3 id="zoom-and-pan">Zoom and pan</h3>
                    <p>Zoom actions can be triggered in different ways. The most common one is to use mouse and scroll wheel. By scrolling up, image is zoomed in, while by scrolling down, image is zoomed out. Alternatively, users may use the tool  buttons at the bottom-right corner of the image viewer to zoom in, zoom out, zoom to fit screen resolution, or zoom to fit image view.</p>
                    <p><img src={zoomButton} style={{ width: "70%", height: "auto" }}/></p>
                        <p>Pan action is achieved by <code>Click-and-drag</code> as default. This default can be changed via the preferences dialogue (<strong>File</strong> -&gt; <strong>Preferences</strong> -&gt; <strong>Global</strong>). The alternative mode is <code>Click</code> and the clicked pixel will be centered in the image viewer.</p>
                        ...
                        <p><img src={WCSMatchButton} style={{ width: "70%", height: "auto" }}/></p>
                            ...
                            <p>Note that images are spatially matched by applying translation, rotation, and scaling. Inconsistency may be prominent when attempting to match wide field images or images with different projection types. However, grid lines are still accurate per image. If contour layers exist, they will match the raster image in the current image view with high position accuracy. Spectral matching is performed with nearest interpolation.</p>
                            ...
                            <p><img src={contourButton} style={{ width: "70%", height: "auto" }}/></p>
                                ...
                                <p><img src={regionButton} style={{ width: "70%", height: "auto" }}/></p>
                                    ...
                                    <p><img src={overlaySettingButton} style={{ width: "70%", height: "auto" }}/></p>
                                        ...
                                        <p><img src={exportPNGButton} style={{ width: "70%", height: "auto" }}/></p>
                                        ...
                                        <p><img src={imageInfoButton} style={{ width: "70%", height: "auto" }}/></p>
            </div>
                                        );
                                    }
                                }
