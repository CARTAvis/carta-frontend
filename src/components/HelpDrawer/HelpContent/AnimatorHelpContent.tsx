...
export const ANIMATOR_HELP_CONTENT = (
    <div>
        ...
        <p>
            The Animator widget grants you control of the rendered context of an image cube in the image viewer. This includes the ability to designate which image, channel (if multiple channels are present within the active image file),
            and polarization component (if the active image file encompasses multiple Stokes parameters) you wish to explore. Moreover, you can have animation playback for any of these selections – image, channel, or polarization – using
            the <code>Play</code> button. To change the animating reference, the accompanying radio buttons serve as your guide. Various playback modes are available:
        </p>
        <ul>
            <li>
        ...
        <p>
            Facilitating channel-specific animation playback, a step value (defaulted at 1) can be established using the <code>Step</code> spinbox. By clicking the <code>Frame rate</code> dropdown, you can opt for the "Step" mode and
            subsequently define the preferred step value via the accompanying spinbox.
        </p>
        <p>
        ...
        <p>
            In order to maintain optimal performance and effective resource utilization, the default behavior entails automatic cessation of animation playback after 5 minutes. However, this parameter can be tailored to your preferences.
            Simply access the Performance tab within the preferences dialog (File -&gt; Preferences) to adjust this setting. The maximum allowable playback duration stands at 30 minutes.
        </p>
    </div>
);
