...
export const IMAGE_FITTING_HELP_CONTENT = (
    <div>
        ...
        <p>
            The image fitting dialog allows you to fit multiple 2D Gaussian components to your image. The default <code>Data Source</code> is the active image which is the current image in the image viewer if it is in single-panel mode or the
            image highlighted with a red box if it is in multi-panel mode. You may select other images as the input image with the <code>Data Source</code> dropdown menu. In the current implementation, the input image data is coupled to the
        ...
        <p>
            ...
            <code>Amplitude</code>, <code>FWHM</code>, and <code>P.A. (deg)</code> (position angle). In the current implementation, center and FWHM are defined in pixels. In a future release, it will also be possible to define these two parameters in
            angular units.
        </p>
        <p>
