    ...
    public render() {
        return (
            <div>
                ...
                <p>The animator widget controls which image, which channel (if it exists per image file), and which Stokes (if exists per image file) to view in the image viewer. You may also enable animation playback for image, channel, or
                    Stokes, via the <code>Play</code> button. The radio buttons control which one to animate. Playback mode includes</p>
                <ul>
                ...
                <p>A step for animation playback (default 1) can be set with the step spinbox. Click the frame rate dropdown to select <code>Step</code> and use the spinbox to define a step.</p>
                <p>For performance reasons and resource management, animation playback will be automatically stopped after 5 minutes by default. This can be customized in the <code>Performance</code> tab of the preferences dialog
