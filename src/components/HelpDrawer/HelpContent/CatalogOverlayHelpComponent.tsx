    ...
    public render() {
        return (
            <div>
                ...
                <p>Source catalog files in VOTable or FITS format can be loaded in CARTA (via <code>File</code>-&gt; <code>Import catalog</code>) for visualization as an image overlay, or a 2D scatter plot, or a histogram.</p>
                ...
                <p>Once filters are set, when the <code>Update</code> button is clicked, the filters will be applied and a filtered source catalog will be displayed up to a number of entries defined in the <code>Max Rows</code> text input field. When
                    the <code>Reset</code> button is clicked, all filters will be removed and the image overlay (if exists) will be removed too. For the histogram plot or the 2D scatter plot, the plot will be reset so that only the first 50
                ...
                <p>Multiple catalog files can be loaded and you may use the <code>File</code> dropdown at the top of the widget to switch in between. Multiple catalog widgets may be launched to display different catalog files.
                    The <code>Close</code> button at the bottom of the widget will close the selected catalog file in the <code>File</code> dropdown. If there are spatially matched images, catalog image overlays are shared between matched images with proper coordinate transformations.</p>
            </div>
        );
    }
}
