    ...
    public render() {
        return (
            <div>
                ...
                <p>The image list widget displays loaded images as a list, which includes image name, rendering layers (R as raster and C as contours) and layer visibility state, WCS matching state, channel index, and Stokes index. The channel index and Stokes index are synchronized with the animator.</p>
                ...
                <p>The list order reflects the order of the image slider in the animator. The order may be modified by selecting an entry by <code>clicking</code> then <code>click-and-dragging</code> to form a desired order.</p>
            </div>
        );
    }
}
