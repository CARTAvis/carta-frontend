    ...
    public render() {
        return (
            <div>
                ...
                <p>When none of the checkboxes is selected, the spectral profiler widget displays one spectrum at a time only. Users can view a region spectral profile (via the &quot;Region&quot; dropdown) of an image cube (via the &quot;Image&quot; dropdown) with a specific statistic (via the &quot;Statistic&quot; dropdown; default as mean). If Stokes axis exists, users may view a specific Stokes via the &quot;Stokes&quot; dropdown. Users may have multiple widgets to view spectra side by side.</p>
                <p>When one of the checkboxes (Image/Region/Statistic/Stokes) is selected, the spectral profiler widget can display multiple spectra in one plot, depending on the selection in the dropdown menu of the selected checkbox. Users can compare different spectra with same x and y ranges directly.</p>
                <p>The spectral profiler widget supports four modes of multi-profile plot:</p>
                <ul>
                    <li>When the &quot;Image&quot; checkbox is selected: spectra from <em>different spatially and spectrally matched images</em>, from a selected region with a selected statistic, and a selected Stokes (if applicable), are displayed.</li>
                    <li>When the &quot;Region&quot; checkbox is selected: spectra from <em>different regions</em> of a selected image (and a selected Stokes if applicable) are displayed. The region spectra are computed with a selected statistic.</li>
                    <li>When the &quot;Statistic&quot; checkbox is selected: spectra with <em>different statistic quantities</em> from a selected region of a selected image (and a selected Stokes if applicable) are displayed.</li>
                    <li>When the &quot;Stokes&quot; checkbox is selected: spectra with <em>different Stokes parameters</em> from a selected image, a selected region and a selected statistic are displayed.</li>
                </ul>
                <p>In short, only one of the four options (Image, Region, Statistic, and Stokes) can be multiple. Others stay as single.</p>
                <p>Cursor information of each profile is displayed at the bottom-left corner. The cursor information box is resizible when necessary.</p>

                <h4>LIMITATION</h4>
                <p>If the intensity units of different matched cubes are different (e.g., Jy/beam vs Kelvin), <em>no</em> unit convesion is applied in the multi-profile plot. Intensity unit conversion will be available in a future release.</p>

                <h3 id="images">Image dropdown menu</h3>
                <p>The image dropdown menu defaults to &quot;Active&quot; image which means the current image in the image viewer. Users may use the animator widget or the image list widget to change the active image.</p>
                <p>When the image checkbox is selected and if there are spatially and spectrally matched images (apply matching via the image list widget), the dropdown menu will displayed an image list with information about the matching state. Users can select one of the images to view its spectral profile. If the selected image is matched to other images, spectra from those images will be displayed in the spectral profiler widget too, allowing a direct comparsion of spectra from different image cubes.</p>   
                <h3 id="regions">Region dropdown menu</h3>
                <p>The region dropdown menu defaults to &quot;Active&quot; region which means a selected region in the image viewer. Users can select an active region by clicking one on the image viewer, or by clicking a region entry in the region list
                    widget. Spectral profile plot of the selected active region will be updated accordingly. If no region is selected, &quot;Active&quot; region defaults to cursor.</p>
                <p>When the region checkbox is selected, the dropdown menu allows multiple-selection. Users can select different regions for profile calculations. </p>
                <h3>Statistic dropdown menu</h3>
                <p>The statistic dropdown menu defaults to &quot;Mean&quot;. When the statistic checkbox is selected, the dropdown menu allows multiple-selection. Users can select different statistic quantities for region spectral profile calculations.</p>
                <h3>Stokes dropdown menu</h3>
                <p>When the image in the view contains multiple Stokes parameters, users can use this dropdown menu to view profiles from different Stokes. The dropdown menu defaults to &quot;Current&quot;, meaning the selected Stokes via the animator.</p>
                <p>When the Stokes checkbox is selected, the dropdown menu allows multiple-selection. Users can select different Stokes parameters for region spectral profile calculations.</p>
                ...
                <p>Profile fitting to a spectrum in the view can be made via the &quot;Fitting&quot; tab of the spectral profiler settings dialogue (the cog icon). A shortcut button of the &quot;Fitting&quot; tab can be found at the top-right corner of the
                ...
                <p>When a region spectral profile is requested, depending on the performance of the server, users may see profiles are updated piece by piece with a regular interval. This feature provides a visual progress update for better
