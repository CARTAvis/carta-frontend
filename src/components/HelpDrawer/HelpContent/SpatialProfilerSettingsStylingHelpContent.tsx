export const SPATIAL_PROFILER_SETTINGS_STYLING_HELP_CONTENT = (
    <div>
        <h3>Styling</h3>
        <p>
            The <code>Styling</code> tab allows you to adjust the appearance of the profile plot, and set x and y ranges of the plot explicitly. In addition, you can select which cut (horizontal or vertical) at the cursor to use to generate a
            spatial profile. You may also enable visualization of mean and RMS values of the current profile in the plot.
        </p>
        <p>Supported options for plot appearance are:</p>
        <ul>
            <li>color of the plot</li>
            <li>plot styles including steps (default), lines, and dots</li>
            <li>line width for steps or lines</li>
            <li>point size for dots</li>
            <li>display alternative horizontal axis in world coordinate</li>
            <li>display mean and RMS</li>
            <li>x and y ranges</li>
        </ul>
    </div>
);
