...
export class ImageViewComponent extends React.Component<WidgetProps> {
    ...
    @observable isShowRatioIndicator: boolean = false;

    onResize = (width: number, height: number) => {
        if (width > 0 && height > 0) {
            const appStore = AppStore.Instance;
            const isRequiresAutoFit = appStore.preferenceStore.zoomMode === Zoom.FIT && appStore.fullViewWidth <= 1 && appStore.fullViewHeight <= 1;
            appStore.setImageViewDimensions(width, height);
            if (isRequiresAutoFit) {
    ...
    constructor(props: WidgetProps) {
        ...
        this.disposers.push(
            autorun(() => {
                ...
                const hasImageSizeChanged = !this.cachedImageSize || this.cachedImageSize.x !== imageSize.x || this.cachedImageSize.y !== imageSize.y;
                const hasGridSizeChanged = !this.cachedGridSize || this.cachedGridSize.x !== imageGridSize.x || this.cachedGridSize.y !== imageGridSize.y;
                if (hasImageSizeChanged || hasGridSizeChanged) {
    ...
    get panels() {
        ...
        return appStore.channelMapStore.isChannelMapEnabled
            ? [<ChannelMapViewComponent isDocked={this.props.isDocked} key="channel-map-panel" />]
            : visibleImages.map((image, index) => {
                  const column = index % config.numImageColumns;
                  const row = Math.floor(index / config.numImageColumns);

                  return <ImagePanelComponent ref={this.collectImagePanelRef} key={`${image?.type}-${image?.store?.id}`} isDocked={this.props.isDocked} image={image} row={row} column={column} />;
              });
    }
