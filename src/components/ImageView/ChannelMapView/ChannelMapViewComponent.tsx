...
export class ChannelMapStore {
    ...
    constructor() {
        ...
        this.numColumns = 2;
        this.numRows = 2;
        this.overlayStores = {corner: undefined, outer: new OverlayStore(0, 0, 0, 0)};
...
export const ChannelMapViewComponent: React.FC<ChannelMapViewComponentProps> = observer((props: ChannelMapViewComponentProps) => {
    ...
    const overlayCanvasRef = React.useRef<any>();
            ...
            <div style={{top: titleOffset + cursorInfoOffset + (frame.overlayStore.colorbar.position === "top" ? colorbarOffset : 0), left: outerOffset, position: "absolute"}}>
                {channelMapStore.channelArray.map((channel, index) => {
                    ...
                    if (column === 0 && row === channelMapStore.numRows - 1) {
                        overlayCanvasRef.current = channelMapStore.overlayStoreCanvasReference.corner;
                    }
                    ...
                    return (
                        <div key={channel}>
                            ...
                            <OverlayComponent
                                ref={ref => {
                                    if (column === 0 && row === channelMapStore.numRows - 1) {
                                        channelMapStore.overlayStoreCanvasReference["corner"] = ref;
                                        if (ref?.canvas) {
                                ...
                                refCanvas={column === 0 && row === channelMapStore.numRows - 1 ? undefined : overlayCanvasRef?.current?.canvas}
                            />
