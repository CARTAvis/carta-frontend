...
export const ChannelMapViewComponent: React.FC<ChannelMapViewComponentProps> = observer((props: ChannelMapViewComponentProps) => {
            ...
            channel < frame?.frameInfo.fileInfoExtended.depth && (
                ...
                >
                    ...
                    <RegionViewComponent
                        ...
                        isDocked={props.isDocked}
                    />
                    {isCornerOverlay && <BeamProfileOverlayComponent frame={frame} top={top} left={left} isDocked={props.isDocked} padding={10} />}
                </div>
            )
        );
    });
    ...
    return (
        ...
        >
            <ChannelMapInnerOverlayComponent frame={frame} isDocked={props.isDocked} />
            {overlayComponents}
            <RasterViewComponent
...
const ChannelMapInnerOverlayComponent = observer(({frame, isDocked}: {frame: FrameStore; isDocked: boolean}) => {
    const appStore = AppStore.Instance;
    const overlaySettings = appStore.overlaySettings;
    const channelMapStore = appStore.channelMapStore;
    const lastRow = Math.floor((channelMapStore.channelArray.length - 1) / channelMapStore.numColumns);
    ...
    const className = classNames("overlay-canvas", {isDocked: isDocked});
