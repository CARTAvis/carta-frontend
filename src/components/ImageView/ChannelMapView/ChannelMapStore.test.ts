...
import {ChannelMapStore} from "./ChannelMapViewComponent.tsx";

const mockConsoleError = jest.spyOn(console, "error");

describe("ChannelmapStore", () => {
    let channelMapStore: ChannelMapStore;
    ...
    beforeEach(() => {
        ...
        channelMapStore = new channelMapStore(0);
    });

    it("initializes the values correctly", () => {
        expect(channelMapStore.startChannel).toBe(0);
        expect(channelMapStore.numColumns).toBe(2);
        expect(channelMapStore.numRows).toBe(2);
        //overlayStores
    });
    ...
    describe("set master frame", () => {
        it("sets the master frame correctly", () => {
            channelMapStore.setMasterFrame(mockFrame);
            expect(channelMapStore.masterFrame).toBe(mockFrame);
        });
    });

    describe("set start channel", () => {
        it("sets channel map start channel correctly", () => {
            channelMapStore.setStartChannel(mockFrame);
            expect(channelMapStore.startChannel).toBe(mockFrame);
        });
        it("fails when start channel is out of invalid for the frame", () => {
            channelMapStore.setStartChannel(mockFrame);
            expect(channelMapStore.startChannel).toBe(mockFrame);
        });
    });
