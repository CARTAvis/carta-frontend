...
export class MomentGeneratorComponent extends React.Component<{widgetStore: SpectralProfileWidgetStore}> {
    ...
    private renderRestFreqInput = (frame: FrameStore) => {
        const isDisableCoordinateSetting = !frame || frame?.isPVImage || !frame?.isSpectralChannel;
        ...
        return (
            <div className="freq-input">
                <ClearableNumericInputComponent
                    ...
                    resetDisabled={restFreqStore?.isResetDisable}
                    tooltipContent={restFreqStore?.defaultInfo}
                    tooltipPlacement={"bottom"}
                />
                <HTMLSelect disabled={isDisableCoordinateSetting} options={Object.values(FrequencyUnit)} value={restFreqStore?.customRestFreq.unit} onChange={ev => restFreqStore?.setCustomUnit(ev.currentTarget.value as FrequencyUnit)} />
            </div>
        );
    };
        ...
        const momentsPanel = (
            <React.Fragment>
                ...
                <FormGroup inline={true} label={"Options"}>
                    ...
                    />
                    {frame === appStore.spatialReference && <Switch label={"Auto spatial matching"} checked={appStore.isMomentToMatch} onChange={appStore.toggleMomentToMatch} />}
                </FormGroup>
