...
import {type DefaultWidgetConfig, LogStore, WidgetProps} from "stores";
    ...
    render() {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            for (let j = 0; j < entry.tags.length; j++) {
                ...
                entryTagSpans.push(
                    <Tag key={j} intent={this.intentFromSeverity(entry.level)}>
                        {entry.tags[j]}
                    </Tag>
                );
                ...
                <div className="log-footer">
                    <FormGroup inline={true} label="Log level:" className="log-level">
                        <HTMLSelect value={logStore.logLevel} onChange={this.onLogLevelChanged} style={{color: this.colorFromSeverity(logStore.logLevel)}}>
                            <option value={CARTA.ErrorSeverity.DEBUG} style={{color: this.colorFromSeverity(CARTA.ErrorSeverity.DEBUG)}}>
                                Debug
                            </option>
                            <option value={CARTA.ErrorSeverity.INFO} style={{color: this.colorFromSeverity(CARTA.ErrorSeverity.INFO)}}>
                                Info
                            </option>
                            <option value={CARTA.ErrorSeverity.WARNING} style={{color: this.colorFromSeverity(CARTA.ErrorSeverity.WARNING)}}>
                                Warning
                            </option>
                            <option value={CARTA.ErrorSeverity.ERROR} style={{color: this.colorFromSeverity(CARTA.ErrorSeverity.ERROR)}}>
                                Error
                            </option>
                            <option value={CARTA.ErrorSeverity.CRITICAL} style={{color: this.colorFromSeverity(CARTA.ErrorSeverity.CRITICAL)}}>
                                Critical
                            </option>
                        </HTMLSelect>
                    </FormGroup>
