...
export class TileService {
    ...
    getTile(tileCoordinateEncoded: number, fileId: number, channel: number, isPeek: boolean = false) {
    ...
    private getRequiredRequestTiles(tiles: TileCoordinate[], fileId: number, channel: number, stokes: number) {
        ...
        for (const tile of tiles) {
            ...
            const isPendingCompressionMap = this.pendingDecompressions.get(key);
            const isTileQueuedForDecompression = isPendingCompressionMap && Array.from(isPendingCompressionMap.values()).some(map => map.has(encodedCoordinate));
            ...
            if (!isTileCached && compressedTile && !isTileQueuedForDecompression) {
                ...
                } else if (!isPendingCompressionMap.has(SINGLE_TILE_DECOMPRESION_SYNC_ID)) {
                    isPendingCompressionMap.set(SINGLE_TILE_DECOMPRESION_SYNC_ID, new Map<number, boolean>());
                }
    ...
    private handleStreamedTiles = (tileMessage: CARTA.IRasterTileData) => {
        ...
        const isPendingCompressionMap = this.pendingDecompressions.get(key)?.has(tileMessage.syncId || 0);
    ...
    private asyncDecompressTile(
        ...
        if (syncId) {
            isPendingCompressionMap.get(syncId as number)?.set(tileCoordinate, true);
        }
    ...
    private updateStream(
        ...
        const key = `${fileId}_${stokes}_${channel}`;
        const isPendingCompressionMap = this.pendingDecompressions.get(key)?.get(syncId || SINGLE_TILE_DECOMPRESION_SYNC_ID);
        if (!isPendingCompressionMap) {
        ...
        if (syncId && syncId > 0 && (this.isAnimationEnabled || pendingTiles?.size)) {
            ...
            isPendingCompressionMap.delete(encodedCoordinate);
