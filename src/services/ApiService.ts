    ...
    public getPreferences = async () => {
        ...
        if (ApiService.RuntimeConfig.apiAddress) {
            try {
                ...
                console.error(err);
                AppStore.Instance.alertStore.shouldShowDashboard(`Failed to load preferences: preferences.json might be malformed and reset to default. Please check the backup file preferences.json.bak.`);
                return undefined;
            }
        } else {
        ...
        if (preferences) {
            ...
            if (!isValid) {
                ...
                if (deletedKeys.length > 0) {
                    ...
                    AppStore.Instance.alertStore.shouldShowDashboard(`Invalid preferences reset to defaults: ${deletedKeys.join(", ")}`);
                }
            }
        }
        return preferences;
