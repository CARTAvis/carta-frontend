...
EMSCRIPTEN_KEEPALIVE AstFrameSet* createOffsetFrameset(AstFrameSet* wcsinfo, double offsetX, double offsetY, double pixelOffsetX, double pixelOffsetY, int skyRefIs)
{
    ...
    if (!astOK) {
        astClearStatus;
        // astAnnul returns a null pointer for this local handle; assigning back prevents accidental reuse.
    ...
    // Set SkyRefIs to Origin or Pole based on the skyRefIs parameter
    if (skyRefIs == 1) {
        astSet(wcsinfoOffset, "SkyRefIs=Pole");
        astSet(wcsinfoOffset, "Label(1)=Offset longitude,Label(2)=Offset colatitude");

        // Remap the sky frame so that the latitude axis shows colatitude:
        // lat' = pi/2 - lat. This keeps the center at 0°.
        ...
        if (!flipLatMap || !shiftLatMap || !colatMap || !astOK) {
            astClearStatus;
            // Release all local references on failure so WASM heap usage does not accumulate.
            if (colatMap) {
