...
export function transformPointList(frameSet: FrameSet, npoint: number, start: {x: number, y: number}, finish: {x: number, y: number});
export function transformAxPointList(frameSet: FrameSet, npoint: number, axis: number, x: number, y: number, dist: number);
