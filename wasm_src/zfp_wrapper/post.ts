...
const zfpDecompress3D = Module.cwrap("zfpDecompress3D", "number", ["number", "number", "number", "number", "number", "number", "number"]);
...
Module.zfpDecompress3DUint8WASM = function (u8: Uint8Array, compressedSize: number, nx: number, ny: number, nz: number, precision: number) {
    let newNumDataBytes = nx * ny * nz * 4;
    if (!Module.dataPtr || newNumDataBytes > Module.nDataBytes) {
        if (Module.dataHeap) {
            Module._free(Module.dataHeap.byteOffset);
        }
...
ctx.onmessage = (event => {
    if (event.data && Array.isArray(event.data) && event.data.length > 1) {
        ...
        if (eventName === "setid") {
        ...
        } else if (eventName.includes("decompress3D")) {
            ...
            if (Module.debugOutput) {
                ...
                const eventSize = (4e-6 * eventArgs.width * eventArgs.subsetHeight * eventArgs.depth);
                setTimeout(() => {
