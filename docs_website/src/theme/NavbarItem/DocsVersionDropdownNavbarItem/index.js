...
function buildApiVersionTargetPath({pathname, apiBasePath, knownSegments, target}) {
    if (!pathname.startsWith(apiBasePath)) {
        return null;
    }
    const relative = pathname.slice(apiBasePath.length);
    const normalizedRelative = relative === "" || relative.startsWith("/") ? relative : `/${relative}`;
    const {matched, remainder} = extractKnownVersionRemainder(normalizedRelative, knownSegments);
...
export default function DocsVersionDropdownNavbarItem({mobile, docsPluginId, dropdownActiveClassDisabled, dropdownItemsBefore, dropdownItemsAfter, versions: configs, ...props}) {
    ...
    if (items.length <= 1) {
        return <DefaultNavbarItem {...props} mobile={mobile} label={dropdownLabel} isActive={dropdownActiveClassDisabled ? () => false : undefined} />;
    }

    return <DropdownNavbarItem {...props} mobile={mobile} label={dropdownLabel} items={items} isActive={dropdownActiveClassDisabled ? () => false : undefined} />;
}
