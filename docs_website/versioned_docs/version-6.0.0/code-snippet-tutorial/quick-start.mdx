...
single argument. <ApiLink path="/stores/class/AppStore/#openFile">`openFile`</ApiLink> must be called with `await`, as it is an asynchronous function that requires communicating with the backend. In the following example, in the end we will
see that only the last image is loaded as each <ApiLink path="/stores/class/AppStore/#openFile">`openFile`</ApiLink> will close all loaded image first before loading the target image.
